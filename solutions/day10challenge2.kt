...
fun main() {
    ...
    println(countArrangements(joltageListAsNums)) // answer
}
...
fun countArrangements(joltageList: List<Int>): Long {
    ...
    val pairList = mutableListOf<Pair<Int, Long>>()

    orderedList.forEachIndexed { index, value ->

        if (index == 0) pairList.add(Pair(value, 1))
        else {
            val sublist = mutableListOf<Pair<Int, Long>>()
            pairList.forEach {
                if (it.first >= value - 3 && it.first < value) {
                    sublist.add(it)
                }
            }
            ...
            pairList.add(Pair(value, total))
        }
    }

    return pairList.last().second
}
